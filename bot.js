...
async function handleMessage(msg) {
    ...
    if (text === "اضافه کردن پروکسی" && await isAdminOrOwner(userId)) {
        db.actions[userId] = "add_proxy_name";
        saveDatabase();
        await sendTelegram("sendMessage", {
            chat_id: chatId,
            text: "✍️ لطفاً نام دلخواه برای پروکسی را ارسال کنید (مثلاً: پروکسی اختصاصی 1 یا پینگ پایین):",
            reply_markup: { keyboard: [[{ text: "🔙 بازگشت" }]], resize_keyboard: true }
        });
        return;
    }
    ...
    if (action) {
        ...
        if (action.startsWith("replace_proxy_")) {
            const proxyIndex = parseInt(action.replace("replace_proxy_", ""));
            const newLink = text.trim();

            if (db.proxies && db.proxies[proxyIndex]) {
                const proxyName = db.proxies[proxyIndex].name;
                ...
                await sendTelegram("sendMessage", {
                    chat_id: chatId,
                    text: `✅ پروکسی جایگزین با موفقیت برای "${proxyName}" ثبت شد!`,
                    reply_markup: { keyboard: [[{ text: "پنل مدیریت" }, { text: "🔙 بازگشت" }]], resize_keyboard: true }
                });
            } else {
                delete db.actions[userId];
        ...
        if (action === "add_proxy_name") {
            const proxyName = text.trim();
            db.actions[`temp_proxy_name_${userId}`] = proxyName;
            db.actions[userId] = "add_proxy_link";
            saveDatabase();
            await sendTelegram("sendMessage", {
                chat_id: chatId,
                text: `🔗 حالا لینک پروکسی را برای نام "${proxyName}" ارسال کنید:`,
                reply_markup: { keyboard: [[{ text: "🔙 بازگشت" }]], resize_keyboard: true }
            });
            return;
        }

        if (action === "add_proxy_link") {
            const proxyLink = text.trim();
            const proxyName = db.actions[`temp_proxy_name_${userId}`] || "پروکسی جدید";
            ...
            delete db.actions[userId];
            delete db.actions[`temp_proxy_name_${userId}`];
            saveDatabase();

            await sendTelegram("sendMessage", {
                chat_id: chatId,
                text: `✅ پروکسی با نام "${proxyName}" در انبار ثبت شد!`,
                reply_markup: { keyboard: [[{ text: "پنل مدیریت" }, { text: "🔙 بازگشت" }]], resize_keyboard: true }
            });
            return;
        }
...
async function handleCallbackQuery(cq) {
    ...
    if (data.startsWith("del_proxy_")) {
        ...
        if (db.proxies && db.proxies[index]) {
            if (!db.actions) db.actions = {};
            db.actions[userId] = `replace_proxy_${index}`;
            saveDatabase();

            await sendTelegram("answerCallbackQuery", { callback_query_id: cq.id, text: "لطفا پروکسی جایگزین را ارسال کنید." });
            await sendTelegram("sendMessage", {
                chat_id: chatId,
                text: `✍️ پروکسی که می‌خواهید جایگزین "${db.proxies[index].name}" شود را بفرستید:`,
                reply_markup: { keyboard: [[{ text: "🔙 بازگشت" }]], resize_keyboard: true }
            });
        } else {
            await sendTelegram("answerCallbackQuery", { callback_query_id: cq.id, text: "⚠️ این پروکسی وجود ندارد." });
